...
<%@ include file="/WEB-INF/back/head.jsp"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<title>jimboshop-list</title>
	...
	<div class="body-box">
		<form action="/product/list.do" method="post"
			...
			<input type="hidden" value="1" name="pageNo" /> 名称: <input
				...
				<option value="1"
					<c:if test="${isShow == 1 }">selected="selected"</c:if>>上架</option>
				<option value="0"
					<c:if test="${isShow == 0 }">selected="selected"</c:if>>下架</option>
		...
		<form method="post" id="tableForm">
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list }" var="entry">
						<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'"
							...
							<td align="center">是</td>
							<td align="center">是</td>
							<td align="center"><c:if test="${isShow == 0 }">下架</c:if>
								<c:if test="${isShow == 1 }">上架</c:if></td>
			...
			<div class="page pb15">
				<span class="r inb_a page_b"> <c:forEach
						items="${pagination.pageView }" var="pageView">
					${pageView }
				</c:forEach>
				</span>
			</div>
			<div style="margin-top: 15px;">
				<input class="del-button" type="button" value="删除"
					...
					value="上架" onclick="optDelete();" /><input class="del-button"
					type="button" value="下架" onclick="optDelete();" />
			</div>
		</form>
	</div>
</body>
